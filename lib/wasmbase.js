...
*/
  return {
    ...
    _printNum: function (n) { console.log('*' + n + '*');}
  };
}
...
function initNativeBase(module) {
  ...
  StringBox.size = baseInstance.exports.__ZN8wasmbase9StringBox8get_sizeEv();
  return baseInstance;
}
...
function getInstance(module, imports, options, ii) {
...
var StringBox = {
  ...
  init: function (ptr, s) {
    if (!s) {
      baseInstance.exports.__ZN8wasmbase9StringBox5resetERS0_(ptr);
      return;
    }
    var size = s.length | 0;
    var bufferPtr = baseInstance.exports.__ZN8wasmbase9StringBox5resetERS0_l(ptr, size);
    var _view = new DataView(Memory.buffer);
    for (var i = 0; i < size; i++)
      _view.setUint16(bufferPtr + (i << 1), s.charCodeAt(i), true);
  },
  destroy: function (ptr) {
    baseInstance.exports.__ZN8wasmbase9StringBox5clearERS0_(ptr);
  },
  get: function (ptr) {
    var _tmp = Memory.malloc(12);
    var charsPtr = _tmp;
    var sizePtr = _tmp + 4;
    var usePtr = _tmp + 8;

    var success = baseInstance.exports.__ZN8wasmbase9StringBox11get_contentERKS0_PlPPKwS3_(ptr, usePtr, charsPtr, sizePtr);
    if (!success) {
      Memory.free(_tmp);
      return null;
    }
    var _view = new DataView(Memory.buffer);
    var p = _view.getUint32(charsPtr, true);
    var size = _view.getUint32(sizePtr, true);
    var buffer = [];
    for (var i = 0; i < size; i++)
      buffer[i] = String.fromCharCode(_view.getUint16(p + (i << 1), true));
    Memory.free(_tmp);
    return buffer.join('');
  },
};
